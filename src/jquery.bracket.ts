...
($ => {
    ...
    class ResultObject {
        constructor(readonly first: [ Score, Score, Score ],
                    readonly second: [ Score, Score, Score ],
    ...
    type MatchCallback = (round: Round,
                          match: MatchResult,
                          seed: number,
    ...
    class MatchResult {
        ...
        private static emptyTeam(source: () => TeamBlock,
                                 sibling: TeamBlock): TeamBlock {
    ...
    function trackHighlighter(teamIndex: number,
    ...
    function defaultRender(container: JQuery,
                           team: string,
                           score: any,
                           state: EntryState): void {
        switch (state) {
    ...
    const winnerMatchSources = (teams: Array<[Option<object>, Option<object>]>,
                                m: number) => (): [MatchSource, MatchSource] => {
    ...
    function prepareWinners(winners: Bracket,
                            teams: Array<[any, any]>,
    ...
    const loserMatchSources = (winners,
                               losers,
                               matchCount: number,
                               m,
                               n,
    ...
    const mkMatchConnector = (centerConnectors: boolean) => (tC,
                                                             match: Match): Connector => {
    ...
    function prepareLosers(winners: Bracket,
                           losers: Bracket,
                           teamCount: number,
                           skipGrandFinalComeback: boolean,
                           centerConnectors: boolean) {
    ...
    function prepareFinals(finals: Bracket,
                           winners: Bracket,
                           losers: Bracket,
                           opts: Options,
                           topCon: JQuery,
    ...
    function teamState(team: TeamBlock,
                       opponent: TeamBlock,
                       score: Score): EntryState {
        return team.name
            .map(() =>
                score
    ...
    class Round {
        private containerWidth = this.opts.teamWidth + 3*this.opts.scoreWidth;
        ...
        public addMatch(teamCb: (() => [MatchSource, MatchSource]) | null,
                        renderCb: Option<RenderCallback>): Match {
    ...
    class Bracket {
        ...
        public addRound(doRenderCb: Option<BoolCallback>): Round {
            ...
            const roundResults = this.initResults.map<ResultObject[]>(
                r =>
                    r[id] === undefined
                        ? new ResultObject([Score.empty(),Score.empty(),Score.empty()], [Score.empty(),Score.empty(),Score.empty()], undefined)
                        : r[id]
            );
    ...
    function mkConnector(roundMargin: number,
                         connector: Connector,
                         teamCon: JQuery,
                         align: string): JQuery {
        ...
        output.results = output.results.map(brackets =>
            brackets.map(rounds =>
                rounds.map((matches: ResultObject) => {
                    const matchData = [[matches.first[0].toNull(),matches.first[1].toNull(),matches.first[2].toNull()], [matches.second[0].toNull(),matches.second[1].toNull(),matches.second[2].toNull()]];
    ...
    function teamElement(roundNumber: number,
                         match: MatchResult,
                         team: TeamBlock,
                         opponent: TeamBlock,
                         isReady: boolean,
        ...
        const resultIdAttribute1 =
            team.name.isEmpty() || opponent.name.isEmpty()
                ? ""
                : `data-resultid="result-${resultId.get()}1"`;
        const resultIdAttribute2 =
            team.name.isEmpty() || opponent.name.isEmpty()
                ? ""
                : `data-resultid="result-${resultId.get()}2"`;
        const resultIdAttribute3 =
            team.name.isEmpty() || opponent.name.isEmpty()
                ? ""
                : `data-resultid="result-${resultId.get()}3"`;
        const teamOrder =
            match.a === team
                ? `first`
                : match.b === team ? `second`
        ...
        // Score 1
        const score1El = $(
            `<div class="score ${teamOrder}" style="width: ${
                opts.scoreWidth
                }px;" ${resultIdAttribute1}></div>`
        );
        ...
        const score1 =
            team.name.isEmpty() || opponent.name.isEmpty() || !isReady
                ? Option.empty()
                : team.score1.map(s => `${s}`);
        const score1String = score1.orElse("--");
        ...
        const score2 =
            team.name.isEmpty() || opponent.name.isEmpty() || !isReady
                ? Option.empty()
                : team.score2.map(s => `${s}`);
        const score2String = score2.orElse("--");
            ...
            if (!team.name.isEmpty() && !opponent.name.isEmpty() && isReady) {
                ...
                score1El.click(function () {
                    ...
                    function editor() {
                        ...
                        const initialScore = team.score1.isEmpty() || !isNumber(team.score1.get()) ? "0" : span.text();
                        const input = $('<input type="text">');
                        ...
                        input.keydown(function (e) {
                            ...
                            if (key === 9 || key === 13 || key === 27) {
                                ...
                                let next = topCon.find(
                                    "div.score[data-resultid=result-" + (rId + 1) + "1]"
                                );
                                if (next.length == 0 || next.hasClass('first')) {
                                    next = topCon.find(
                                        "div.score[data-resultid=result-" + (rId - 1) + "2]"
                                    );
                                }
                                if (next) {
                                    next.click();
                ...
                score2El.click(function () {
                    ...
                    function editor() {
                        ...
                        const initialScore = team.score2.isEmpty() || !isNumber(team.score2.get()) ? "0" : span.text();
                        const input = $('<input type="text">');
                        ...
                        input.keydown(function (e) {
                            ...
                            if (key === 9 || key === 13 || key === 27) {
                                ...
                                if (next.length == 0 || next.hasClass('first')) {
                                    next = topCon.find(
                                        "div.score[data-resultid=result-" + (rId - 1) + "3]"
                                    );
                                }
                                if (next) {
                                    next.click();
                ...
                score3El.click(function () {
                    ...
                    function editor() {
                        ...
                        const initialScore = team.score3.isEmpty() || !isNumber(team.score3.get()) ? "0" : span.text();
                        const input = $('<input type="text">');
    ...
    class Match {
        ...
        constructor(private round: Round,
            ...
            match.a.score1 = results.map(r => r.first[0].toNull());
            match.a.score2 = results.map(r => r.first[1].toNull());
            match.a.score3 = results.map(r => r.first[2].toNull());
            match.b.score1 = results.map(r => r.second[0].toNull());
            match.b.score2 = results.map(r => r.second[1].toNull());
            match.b.score3 = results.map(r => r.second[2].toNull());
        ...
        public results(): ResultObject {
            ...
            return new ResultObject(
                [this.match.a.score1,this.match.a.score2,this.match.a.score3],
                [this.match.b.score1,this.match.b.score2,this.match.b.score3],
                this.matchUserData
            );
        }
    }
            ...
            brackets.map(rounds =>
                rounds.map(
                    (matches: [number, number, any]) =>
                        new ResultObject(
                            [Score.of(undefinedToNull(matches[0][0])),Score.of(undefinedToNull(matches[0][1])),Score.of(undefinedToNull(matches[0][2]))],
                            [Score.of(undefinedToNull(matches[1][0])),Score.of(undefinedToNull(matches[1][1])),Score.of(undefinedToNull(matches[1][2]))],
                            matches[2]
                        )
                )
            )
    ...
    const JqueryBracket = (opts: Options) => {
        ...
        function resizeContainer() {
            ...
            if (!opts.disableToolbar) {
                topCon.css(
                    "width",
                    roundCount * (opts.teamWidth + 3*opts.scoreWidth + opts.roundMargin) +
                    40
                );
            } else {
                topCon.css(
